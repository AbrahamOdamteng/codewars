...
namespace CodeWars.kata._8kyu
{
    ...
    /// the last bus station(after the last array). Even though it is the last bus stop,
    /// the bus is not empty and some people are still in the bus, and they are
    /// probably sleeping there :D Take a look on the test cases.
    ...
    class NumberOfPeopleInTheBus
    {
        ...
        {
            return peopleListInOut.Aggregate(0, (onBus, arr) =>
            {
                onBus += arr[0];
                onBus -= arr[1];
                return onBus;
